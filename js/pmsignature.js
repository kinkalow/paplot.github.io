pmsignature = (function () {
  ...
  p.box_template = (function () {
    ...
    p_sub.initialize = function (parent) {
      // rect
      parent.svg_obj.append("g").attr("class", this.name).selectAll("rect").data([]).enter();
  ...
  p.resize = function () {
    ...
    var lineFunction = d3.svg
      .line()
      .x(function (d) {
        return d.x * arrow_w + arrow_x;
      })
      .y(function (d) {
        return d.y * arrow_h + arrow_y;
      })
      .interpolate("linear");
    ...
    this.svg_obj
      ...
      .selectAll("path")
      .attr("d", function (d) {
        return lineFunction(that.arrow.path) + "Z";
      });
    ...
    lineFunction = d3.svg
      .line()
      .x(function (d) {
        return d.x * rect_w + rect_x;
      })
      .y(function (d) {
        return d.y * rect_h + rect_y;
      })
      .interpolate("linear");
    ...
    this.svg_obj
      ...
      .selectAll("path")
      .attr("d", function (d) {
        return lineFunction(that.rect.path) + "Z";
      });
  ...
  p.update = function () {
    ...
    if (this.strand.data.length == 2) {
      ...
      this.svg_obj.selectAll("g.strand").selectAll("rect").remove();
      ...
      // label
      this.svg_obj.selectAll("g.strand_label").selectAll("text").remove();
  ...
  p.draw = function () {
    ...
    // strand
    if (this.strand.data.length == 2) {
      this.svg_obj.append("g").attr("class", "strand").selectAll("rect").data([]).enter();
      // label
      this.svg_obj.append("g").attr("class", "strand_label").selectAll("line").data([]).enter();
    }
    // arrow
    this.svg_obj.append("g").attr("class", "arrow").append("path").attr("d", []).attr("fill", this.arrow.colorset);
