legend = (function () {
  ...
  p.draw_svg = function (display) {
    ...
    // ordinal
    var ordinal = d3.scale.ordinal().domain(p_data.items).range(p_data.colors);

    var legend_color = d3.legend
      .color()
    ...
    if (this.options.horizon == true) {
      // horizon
      legend_color.scale(ordinal).orient("horizontal");
    } else {
      // virtical
      legend_color.labelAlign("start").scale(ordinal);
    }
    ...
    if (isNode == false) {
      legend_svg.select(".legend_class").call(legend_color);
    }
    ...
    if (this.options.horizon == true) {
      ...
      legend_svg
        ...
        .attr(
          "transform",
          "translate(" +
            (this.layout.shape_sift_left + p_data.rect_width + this.layout.rect_margin_right + this.layout.text_sift_left) +
            ", 12)"
        );
    }
    if (this.options.title != "") {
  ...
  p.preparation = function (mode) {
    ...
    if (svg_width == 0) {
      var item_w =
        rect_width +
        this.layout.rect_margin_right +
        this.layout.text_sift_left +
        max_length * this.layout.text_font_size +
        this.layout.text_sift_right;
      if (this.options.horizon == true) {
        if (mode == "svg") {
    ...
    if (svg_height == 0) {
      if (this.options.horizon == true) {
        ...
        svg_height =
          this.layout.padding_top + title_height + items.length * (this.layout.rect_height + this.layout.shape_padding) + 10;
      }
    }
