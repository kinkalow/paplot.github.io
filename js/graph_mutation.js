(function () {
  ...
  var BAR_LEFT_AXIS_Y =
    (Number(style_mut.title_gene_y1_font_size.replace("px", "")) + //
      Number(style_mut.title_gene_y2_font_size.replace("px", ""))) *
    2; //40
  ...
  function update_gene_listbox(genes) {
    ...
    d3.select("#gene_list").selectAll("option").remove();
  ...
  mut_draw.init = function () {
    ...
    div_mut_bar_left.options.titles[1].sift_y =
      Number(style_mut.title_gene_y1_font_size.replace("px", "")) * -0.2 -
      Number(style_mut.title_gene_y2_font_size.replace("px", "")); //-20;
    ...
    for (var i6 = 0; i6 < divs_sub.length; i6++) {
      var sub_legend_w = Number(
        d3
          .select("#" + divs_sub[i6].id + "_l_svg")
          .style("width")
          .replace("px", "")
      );
